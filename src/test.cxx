...
namespace {
  std::vector< rave::Track > createTracks()
  {
    using namespace std;
...
int main(void)
{
  ...
  std::vector < rave::Track > tracks = createTracks();

  std::cout << "[rave factory]" << std::endl;
  std::vector < rave::Vertex > rvertices = factory.create ( tracks, "mvf" );
  for ( auto r=rvertices.begin(); 
        r!=rvertices.end() ; ++r )
  {
    std::cout << r->position() << std::endl;
  }
  std::cout << "[kinematics01] ends" << std::endl;
  return 0;
}
